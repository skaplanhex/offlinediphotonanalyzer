...
// Wed Jul 27 15:34:43 2016 by ROOT version 6.02/13
// from TTree fTightFakeTree/PhotonTightFakeTree
...
#ifndef fTightFakeTree_h
#define fTightFakeTree_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   fTightFakeTree(TTree *tree=0);
   virtual ~fTightFakeTree();
...
#ifdef fTightFakeTree_cxx
fTightFakeTree::fTightFakeTree(TTree *tree) : fChain(0) 
{
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      ...
      dir->GetObject("fTightFakeTree",tree);

   }
   Init(tree);
}

fTightFakeTree::~fTightFakeTree()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t fTightFakeTree::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t fTightFakeTree::LoadTree(Long64_t entry)
{
...
void fTightFakeTree::Init(TTree *tree)
{
...
void fTightFakeTree::Show(Long64_t entry)
{
...
Int_t fTightFakeTree::Cut(Long64_t entry)
{
...
#endif // #ifdef fTightFakeTree_cxx
