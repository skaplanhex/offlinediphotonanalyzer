...
outfile = TFile("thetainput_trial.root","recreate")

# first write signal
msVals = [3000,3500,4000,4500,5000,5500,6000]
for ms in msVals:
    sigFileName = "ADDMs%i_signal.root"%ms
...
bkgFile = TFile("GGJetsPlots_withKFactor_Lumi2p6.root","read")
bkgFile.cd()
...
fakeFile = TFile("fakePlotsNew.root","read")
fakeFile.cd()
...
dataFile = TFile("data2015plots_new.root","read")
dataFile.cd()
